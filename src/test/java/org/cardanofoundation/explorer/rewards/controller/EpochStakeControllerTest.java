...
class EpochStakeControllerTest {
  ...
  @MockBean private EpochStakeConcurrentFetching mockEpochStakeConcurrentFetching;
  ...
  void fetchEpochStakes_Success_ReturnsTrue() throws Exception {
    ...
    when(mockEpochStakeConcurrentFetching.fetchDataConcurrently(stakeAddressList)).thenReturn(true);
    mockMvc
        .perform(
            post("/api/v1/epoch-stake/fetch")
