...
class PoolInfoControllerTest {
  ...
  @MockBean private PoolInfoConcurrentFetching mockPoolInfoConcurrentFetching;
  ...
  void fetchPoolInfoList_Success_ReturnsTrue() throws Exception {
    ...
    when(mockPoolInfoConcurrentFetching.fetchDataConcurrently(poolIdList)).thenReturn(true);
    mockMvc
        .perform(
            post("/api/v1/pool-info/fetch")
