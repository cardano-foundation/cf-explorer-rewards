...
class RewardFetchingServiceImplTest {
  ...
  @Mock private RewardCheckpointRepository rewardCheckpointRepository;

  @Mock private EpochService epochService;

  @Mock private JOOQRewardRepository jooqRewardRepository;

  @Mock private JOOQRewardCheckpointRepository jooqRewardCheckpointRepository;

  @InjectMocks private RewardFetchingServiceImpl rewardFetchingServiceImpl;

  @Captor ArgumentCaptor<List<Reward>> rewardCaptor;

  @Captor ArgumentCaptor<List<RewardCheckpoint>> checkpointCaptor;
  ...
  void fetchData_shouldFetchAndSaveRewardData() throws Exception {
    ...
    accountRewards1.setRewards(List.of(inner1));
    ...
    accountRewards2.setRewards(List.of(inner2));

    accountRewards.add(accountRewards1);
    accountRewards.add(accountRewards2);

    when(koiosClient.accountService().getAccountRewards(anyList(), any(), any()).getValue())
        .thenReturn(accountRewards);
    ...
    when(rewardCheckpointRepository.findByStakeAddressIn(any()))
        .thenReturn(
            List.of(
  ...
  void getStakeAddressListNeedFetchData_shouldReturnStakeAddressList() throws Exception {
    ...
    // Run the test
    List<String> result =
        rewardFetchingServiceImpl.getStakeAddressListNeedFetchData(stakeAddressList);
    // Verify
