...
class JOOQEpochStakeCheckPointRepositoryTest extends TestDataBaseContainer {
  ...
  void saveAll_shouldSaveSuccessWhenOnConflictView() {
    ...
    assertEquals(
        1, dsl.fetchCount(table(tableName), field(viewField).eq(stakeAddress)));
  }
  ...
  void saveAll_shouldSaveSuccessWhenDoesNotOnConflictView() {
    ...
    assertEquals(
        1, dsl.fetchCount(table(tableName), field(viewField).eq("stakeKey1")));
    assertEquals(
        1, dsl.fetchCount(table(tableName), field(viewField).eq("stakeKey2")));
    assertEquals(
        1, dsl.fetchCount(table(tableName), field(viewField).eq("stakeKey3")));
  }
}
